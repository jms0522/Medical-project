...
for department, code in hidak_all.items():
    # 어제의 JSON 파일 이름 생성
    yesterday_file_name = f"{code}_{department}_{yesterday_str}.json"

    if os.path.exists(yesterday_file_name):
        with open(yesterday_file_name, 'r', encoding='utf-8') as f:
            yesterday_qna = json.load(f)
    else:
        print(f"{department} 어제의 데이터가 없습니다.")
        continue
    ...
    new_links = list(set(link_list) - set(yesterday_qna.get(department, [])))
    
    today_qna = {department: []}

    if new_links:
        today_qna[department].extend(new_links)
