...
def scrape_details(doc_id):
    ...
    try:
        ...
        data = {
            ...
            'question': soup.select_one('.c-heading__content').text.strip() if soup.select_one('.c-heading__content') else 'N/A',
            'answer': soup.select_one('.se-main-container').text.strip() if soup.select_one('.se-main-container') else 'N/A',
        }
        
        # print("Extracted Data:", data) 디버깅 용
        
        csv_file_name = 'details.csv'  # 예시 파일 이름
        csv_file_path = os.path.join('/Users/jangminsoo/Desktop/dev/pro/Medical-project/csv_folder/today_naver_QnA', csv_file_name)
